...
public class LyricView extends ScrollView {
    ...
    LinearLayout lyricList;//垂直布局
    List<TextView> lyricItems = new ArrayList<>();//每项的歌词集合
    List<String> lyricTextList = new ArrayList<String>();//每行歌词文本集合，建议先去看看手机音乐里的歌词格式和内容
    List<Long> lyricTimeList = new ArrayList<Long>();//每行歌词所对应的时间集合
    List<Integer> lyricItemHeights;//每行歌词TextView所要显示的高度
    ...
    private void refreshRootView() {
        ...
        if (lyricList != null) {
            rootView.addView(lyricList);//加入一个歌词显示布局
     ...
     * @param timeList
     */
    public void setLyricText(List<String> textList, List<Long> timeList) {
